            ...
            if (SendTransaction(GetPaymentId(Sender), Transfers, TotalAmount, out string TransactionHash))
            {
                ...
                foreach (ulong UID in Recipients)
                {
                    ...
                    if (GetRedirect(UID))
                    {
                        ...
                        ReplyEmbed.Description = string.Format("You recieved a tip of **{0:N4}** {1} from {2}, since you are redirecting tips back to " +
                    ...
                    else ReplyEmbed.Description = string.Format("You recieved a tip of **{0:N4}** {1} from {2}!\nTX: **{3}**", Amount, coinSymbol,
                        _client.GetUser(Sender).Username, TransactionHash);
                ...
                if (Recipients.Count == 1)
                    Response.Description = string.Format("You sent a tip of **{0:N4}** {1} to 1 user\nNew balance: **{2:N4}** {1}\nTX: **{3}**",
                        Amount, coinSymbol, Balance, TransactionHash);
                else Response.Description = string.Format("You sent a tip of **{0:N4}** {1} to {2} users\nNew balance: **{3:N4}** {1}\nTX: **{4}**",
        ...
        public static bool Tip(ulong Sender, string Recipient, decimal Amount)
        {
            ...
            if (SendTransaction(GetPaymentId(Sender), Transfers, TotalAmount, out string TransactionHash))
            {
                ...
                if (GetAddress(Sender) == Recipient)
                {
                    ...
                    Response.Description = string.Format("You withdrew **{0:N4}** {1} to {2}\nNew balance: **{3:N4}** {1}\nTX: **{4}**",
                        Amount, coinSymbol, Recipient, Balance, TransactionHash);
                }
                else
                {
                    Response.WithTitle("Tip sent!");
                    Response.Description = string.Format("You sent a tip of **{0:N4}** {1} to {2}\nNew balance: **{3:N4}** {1}\nTX: **{4}**",
                        Amount, coinSymbol, Recipient, Balance, TransactionHash);
                }
                    ...
                    foreach (KeyValuePair<string, JObject> ConfirmedTransaction in NewTransactions)
                    {
                        ...
                        if (!CheckTransactionExists(ConfirmedTransaction.Key))
                        {
                            ...
                            if (UserExists && Type == "IN")
                            {
                                ...
                                Response.Description = string.Format("Your deposit of **{0:N4}** {1} has now been credited.\nNew balance: **{2:N4}** {1}\nTX: **{3}**",
                        ...
                        else if (CheckIfPending(ConfirmedTransaction.Key))
                        {
                            ...
                            foreach (string PID in PaymentIds)
                            {
                                ...
                                Response.Description = string.Format("A tip of **{0:N4}** {1} that was sent to you has now been credited.\nNew balance: **{2:N4}** {1}\nTX: **{3}**",
